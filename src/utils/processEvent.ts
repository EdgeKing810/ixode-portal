...
export const processEvent = (
  ...
  if (e.event_type === 'user_register') {
    console.log(e.description);
  }

  let event = { ...e };

  event.timestamp = new Date(event.timestamp).toDateString();
  event.timestamp = event.timestamp.toLocaleString();
  event.redirect = event.redirect ? event.redirect : '/';

  if (eventData && eventData.length > 0) {
    let foundEventData = eventData.find((ed) => ed.type === event.event_type);

    if (foundEventData && foundEventData.logo) {
      event.icons = foundEventData.logo.split(';');
    }
  }
  ...
  for (let i = 0; i < textToSplit.length; i++) {
    let tmpIndex =
      textToSplit.slice(0, i + 1).filter((tts) => tts === textToSplit[i])
        .length - 1;
    ...
    if (splf.length > 1 && splf[0].length < 2) {
      let id = splf[1];
