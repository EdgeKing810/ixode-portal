...
export default function Route() {
  ...
  const [currentBlocks, setCurrentBlocks] = useState<Array<IComplexBlock>>([]);
  const [targetBlock, setTargetBlock] = useState(targets[0].name);
  const [currentKdl, setCurrentKdl] = useState('');
  ...
  const isCreating =
    mode !== undefined && ['create', 'c'].includes(mode.toLowerCase());
  ...
  const toggleKdl = (toggle: boolean) => {
    if (kdl) {
      if (!toggle) {
      ...
      >
        ...
        <div className="w-full lg:p-8 flex flex-col h-full">
          <div className="w-full h-full lg:border-2 lg:border-primary lg:p-8 rounded lg:border-opacity-25 lg:overflow-y-scroll">
            ...
            {currentProject && currentProject.id && currentRoute && (
              <RouteBlockDisplay
                API_URL={API_URL}
                profile={profile}
