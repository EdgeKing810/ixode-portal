...
export const Heading = ({ title, children, className }) => (
  <div
    className={`text-main-primary w-full text-2xl sm:text-4xl font-bold font-spartan break-all mb-2 ${className}`}
    title={title}
  >
    {children}
  </div>
...
export const Input = ({
  ...
  <div
    className={`w-full lg:w-1/2 rounded-lg opacity-75 ${!noPadding && 'p-2'} ${
      !noBorder &&
      `${
        borderColor ? `border-main-${borderColor}` : 'border-main-300'
      } border-2`
...
export const PasswordInput = ({
  ...
  >
    ...
    >
      ...
      <button
        className={`outline-none w-8 h-10 sm:w-10 sm:h-10 rounded-lg flex justify-center items-center ${
          theme === 'light' ? 'text-main-darkbg' : 'text-main-lightbg'
        } opacity-85`}
        onClick={(e) => {
          e.preventDefault();
...
export const Button = ({
  ...
  <button
    ...
    className={`font-noto tracking-wide ${
      color ? `text-main-${color}` : 'text-main-primary'
    } text-opacity-75 my-2 flex items-center ${
      transparent
        ? 'bg-transparent'
        : `${theme === 'light' ? 'bg-main-light' : 'bg-main-dark'}`
